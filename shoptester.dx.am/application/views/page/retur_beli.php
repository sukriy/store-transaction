					...
					<div class="col-md-12 col-sm-12 col-xs-12">
						<div class="x_panel">
							<div class="x_title">						
								...
								?>
								<?=$this->session->flashdata('retur_beli');?>
										...
										<?php
											foreach($load as $key=>$value){
												...
												<tr role='row'>
													...
													<td>".cetak($value->supplier)."</td>
													<td>".cetak_tgl($value->tgl_retur_beli)."</td>
...
<script type="text/javascript">
	...
	$('.example tbody').on('click', '.print', function (e) {
		...
		$.post("<?=base_url('Admin/ajax_retur_beli_print'); ?>", {'id_retur_beli' : $(this).closest('tr').find("td").eq(1).text(), <?php echo $this->security->get_csrf_token_name(); ?>: $("input[name=<?php echo $this->security->get_csrf_token_name(); ?>]").val()}, function(result){
	...
	$('.example tbody').on('click', '.edit', function (e) {
		e.preventDefault();
		window.location.replace("<?=base_url('Admin/retur_beli_modif?id_retur_beli=');?>"+$(this).closest('tr').find("td").eq(1).text());
	})
	$('.example tbody').on('click', '.hapus', function (e) {
		...
		swal({
			...
			if (willDelete) {
				...
				$.post("<?=base_url('Admin/ajax_retur_beli_hapus'); ?>", {'hapus' : 1, 'id_retur_beli' : $(this).closest('tr').find("td").eq(1).text(), <?php echo $this->security->get_csrf_token_name(); ?>: $("input[name=<?php echo $this->security->get_csrf_token_name(); ?>]").val()}, function(result){
					var result = JSON.parse(result);
					// console.log(result);
					$("input[name=<?php echo $this->security->get_csrf_token_name(); ?>]").each(function() { 
						$(this).val(result.csrf.hash);
	...
	$('button[name=tambah]').click(function(e){
		e.preventDefault();
		window.location.replace("<?=base_url('Admin/retur_beli_modif');?>");
	});
</script>