									...
									<?php
										foreach($load as $key=>$value){
											...
											if(cetak($value->gambar)==''){
												...
												$base = base_url();
												$gambar="<a class='image-link' href='".$base."images/person/".cetak($value->gambar)."'><span class='glyphicon glyphicon-picture' aria-hidden='true'></span></a>";
											}										
											echo "
											<tr>
												<td>".cetak($value->id_person)."</td>
												<td>".cetak($value->nama_person)."</td>
...
<script type="text/javascript">
	$('.example tbody').on('click', '.edit', function (e) {
		e.preventDefault();
		window.location.replace("<?=base_url('Admin/person_modif?id_person=');?>"+$(this).closest('tr').find("td:first").text());
	})
	$('.example tbody').on('click', '.hapus', function (e) {
		...
		swal({
			...
			if (willDelete) {
				...
				$.post("<?=base_url('Admin/ajax_person_hapus'); ?>", {'hapus' : 1, 'id_person' : $(this).closest('tr').find("td:first").text(), <?php echo $this->security->get_csrf_token_name(); ?>: $("input[name=<?php echo $this->security->get_csrf_token_name(); ?>]").val()}, function(result){
					location.reload();
				});	
			}
		});	
	...
	$('button[name=tambah]').click(function(e){
		e.preventDefault();
		window.location.replace("<?=base_url('Admin/person_modif');?>");
	});
</script>