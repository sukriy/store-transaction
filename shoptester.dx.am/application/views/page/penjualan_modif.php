...
<?php
	include('assets.php');
	if(isset($load)){
		$date1=date_create($load[0]->tgl_penjualan);
				...
				<div class="">
					<div class="page-title">
						<div class="title_left">
							<h3>PENJUALAN <?=(isset($load[0]->id_penjualan) ? 'EDIT' : 'INPUT BARU'); ?></h3>
						</div>
							...
							<div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right top_search">
								<div class="input-group">								
									<ol class="breadcrumb text-right">
										...
										<li><a href="<?=base_url('Admin/penjualan');?>">Penjualan</a></li>
					...
					<div class="row">
						<div class="col-md-12 col-sm-12 col-xs-12">
							<div class="x_panel">
								<div class="x_content">
									<br />
									<?=$this->session->flashdata('penjualan_modif');?>
									<?=$this->session->flashdata('message');?>
									<?=((validation_errors()!='') ? '<div class="alert alert-danger" role="alert">'.validation_errors().'</div>' : ''); ?>
									<?=form_open_multipart('Admin/penjualan_Modif/','id="form_temp"');?> 
										<input type='hidden' class='form-control' name='uniqid' value='<?=uniqid(); ?>'>
										<input type="hidden" class="form-control" name='id_penjualan' value='<?=(isset($load[0]->id_penjualan) ? cetak($load[0]->id_penjualan) :(isset($_POST['id_penjualan']) ? cetak($_POST['id_penjualan']) : (isset($_GET['id_penjualan']) ? cetak($_GET['id_penjualan']) : ''))); ?>'>
										<input type="hidden" id='token' name="<?=$this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>" />
										
										<div class="form-group row">
											<label class="control-label col-md-2 col-sm-2 col-xs-12" for="tgl_penjualan">Tgl Penjualan</label>
											<div class="col-md-4 col-sm-4 col-xs-12">
												<input type="text" class="form-control tgl" autocomplete="off" name='tgl_penjualan' value='<?=(isset($load[0]->tgl_penjualan) ? cetak_tgl($load[0]->tgl_penjualan) : (isset($_POST['tgl_penjualan']) ? $_POST['tgl_penjualan'] : '')); ?>' 
										...
										<div class="form-group row">
											<label class="control-label col-md-2 col-sm-2 col-xs-12" for="customer">Customer</label>
											<div class="col-md-4 col-sm-4 col-xs-12">
												<select data-placeholder="Pilih Customer..." class="form-control standardSelect" name='id_customer'>
													<option value=""></option>
													<?php
										...
										<div class="form-group row">
											<label class="control-label col-md-2 col-sm-2 col-xs-12" for="sales">Sales</label>
											<div class="col-md-4 col-sm-4 col-xs-12">
												<select data-placeholder="Pilih Sales..." class="form-control standardSelect" name='id_sales'>
													<option value=""></option>
													<?php
														foreach($sales->result() as $row){
															echo "<option value='".cetak($row->id_person)."' ".((isset($load[0]->id_sales) && cetak($load[0]->id_sales)==cetak($row->id_person)) ? 'selected' : ((isset($_POST['id_sales']) && cetak($_POST['id_sales'])==cetak($row->id_person)) ? 'selected' : '')).">".cetak($row->nama_person)."</option>";
														}
													?>
												</select>
											</div>
												...
												<select data-placeholder="Pilih Item..." class="form-control standardSelect" tabindex="1" name='item0'>
													...
													<?php
														...
														foreach($item as $key=>$value){
															echo "<option value='".cetak($value['id_item'])."'>".cetak($value['nama'])." (".number_format($value['harga'],0).") - ".cetak($value['stock_sisa'])."</option>";
															$newdata =  array (
														...
														if(isset($_POST['id_item'])){
															...
															foreach($_POST['id_item'] as $key=>$value){
																if(isset($_POST['id_penjualan']) && $_POST['id_penjualan']!=''){
																	$newdata =  array (
																		'id_penjualan' => cetak0($_POST['id_penjualan']),
										...
										<div class="form-group">
											<button type="submit" class="btn btn-success control-label col-md-10 col-sm-10 col-xs-12" name='submit' value=<?=(isset($load[0]->id_penjualan) ? 'edit' : 'baru');?>><?=(isset($load[0]->id_penjualan) ? 'EDIT' : 'INPUT BARU'); ?></button>
										</div>
									</form>
								</div>
							</div>
