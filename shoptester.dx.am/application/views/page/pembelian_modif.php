...
<?php
	include('assets.php');
	if(isset($load)){
		$date1=date_create($load[0]->tgl_pembelian);
				...
				<div class="">
					<div class="page-title">
						<div class="title_left">
							<h3>Pembelian <?=(isset($load[0]->id_pembelian) ? 'EDIT' : 'INPUT BARU'); ?></h3>
						</div>
						...
						<div class="col-md-12 col-sm-12 col-xs-12">
							<div class="x_panel">
								<div class="x_content">
									...
									<?=form_open_multipart('Admin/Pembelian_Modif/','id="form_temp"');?> 
										<input type='hidden' class='form-control' name='uniqid' value='<?=uniqid(); ?>'>
										<input type="hidden" class="form-control" name='id_pembelian' value='<?=(isset($load[0]->id_pembelian) ? cetak($load[0]->id_pembelian) :(isset($_POST['id_pembelian']) ? cetak($_POST['id_pembelian']) : (isset($_GET['id_pembelian']) ? cetak($_GET['id_pembelian']) : ''))); ?>'>
										<input type="hidden" id='token' name="<?=$this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>" />
										
										<div class="form-group row">
											<label class="control-label col-md-2 col-sm-2 col-xs-12" for="tgl_pembelian">Tgl pembelian</label>
											<div class="col-md-4 col-sm-4 col-xs-12">
												<input type="text" class="form-control tgl" autocomplete="off" name='tgl_pembelian' value='<?=(isset($load[0]->tgl_pembelian) ? cetak_tgl($load[0]->tgl_pembelian) : (isset($_POST['tgl_pembelian']) ? $_POST['tgl_pembelian'] : '')); ?>' 
										...
										<div class="form-group row">
											<label class="control-label col-md-2 col-sm-2 col-xs-12" for="tgl_pembelian">Supplier</label>
											<div class="col-md-4 col-sm-4 col-xs-12">
										...
										<div class="form-group row">
											<label class="control-label col-md-2 col-sm-2 col-xs-12" for="no_bon">Nomor Bon</label>
											<div class="col-md-4 col-sm-4 col-xs-12">
												<input type="text" class="form-control" name='no_bon' value='<?=(isset($load[0]->no_bon) ? cetak($load[0]->no_bon) : (isset($_POST['no_bon']) ? cetak($_POST['no_bon']) : '')); ?>' 
													...
													if(isset($load) || isset($_POST['id_item'])){
														if(isset($_POST['id_item'])){
															...
															foreach($_POST['id_item'] as $key=>$value){
																if(isset($_POST['id_pembelian']) && $_POST['id_pembelian']!=''){
																	$newdata =  array (
																		'id_pembelian' => cetak0($_POST['id_pembelian']),
										...
										<div class="form-group">
											<button type="submit" class="btn btn-success control-label col-md-10 col-sm-10 col-xs-12" name='submit' value=<?=(isset($load[0]->id_pembelian) ? 'edit' : 'baru');?>><?=(isset($load[0]->id_pembelian) ? 'EDIT' : 'INPUT BARU'); ?></button>
										</div>
									</form>
								</div>
							</div>
