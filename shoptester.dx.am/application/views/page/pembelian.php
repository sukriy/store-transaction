					...
					<div class="col-md-12 col-sm-12 col-xs-12">
						<div class="x_panel">
							<div class="x_title">						
								...
								?>
								<?=$this->session->flashdata('pembelian');?>
										...
										<?php
											foreach($load as $key=>$value){
												...
												<tr role='row'>
													...
													<td>".cetak_tgl($value->tgl_pembelian)."</td>
													<td>".cetak($value->no_bon)."</td>
...
<script type="text/javascript">
	...
	$('.example tbody').on('click', '.edit', function (e) {
		e.preventDefault();
		window.location.replace("<?=base_url('Admin/pembelian_modif?id_pembelian=');?>"+$(this).closest('tr').find("td").eq(1).text());
	})
	$('.example tbody').on('click', '.hapus', function (e) {
		...
		swal({
			...
			if (willDelete) {
				...
				$.post("<?=base_url('Admin/ajax_pembelian_hapus'); ?>", {'hapus' : 1, 'id_pembelian' : $(this).closest('tr').find("td").eq(1).text(), <?php echo $this->security->get_csrf_token_name(); ?>: $("input[name=<?php echo $this->security->get_csrf_token_name(); ?>]").val()}, function(result){
	...
	$('button[name=tambah]').click(function(e){
		e.preventDefault();
		window.location.replace("<?=base_url('Admin/pembelian_modif');?>");
	});
</script>