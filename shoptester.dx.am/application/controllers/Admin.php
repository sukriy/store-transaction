...
class Admin extends CI_Controller
{
    ...
    public function item_modif()
    {
        ...
        $data['item']=$this->db1->list_nama_item();
    ...
    public function pembayaran()
    {
        ...
        if (isset($_POST['jenis'])) {
            $this->form_validation->set_rules('jenis', 'Jenis', 'required|trim|in_list[pembelian,penjualan]');
            $this->form_validation->set_rules('dari', 'Tanggal Dari', 'trim|callback_check_tgl');
            $this->form_validation->set_rules('sampai', 'Tanggal Sampai', 'trim|callback_check_tgl');
            if ($this->form_validation->run() == true) {
                if ($_SESSION['form_key']!=$_POST['uniqid']) {
                    $_SESSION['form_key']=$_POST['uniqid'];
            ...
            if ($this->form_validation->run() == true) {
                if ($_SESSION['form_key']!=$_POST['uniqid']) {
                    ...
                    if ($pembayaran_baru) {
                        ...
                        $this->session->set_flashdata('pembayaran', '<div class="alert alert-danger text-center">Terjadi Kesalahan</div>');
                        redirect('Admin/pembayaran');
                    }
                } else {
                    redirect('Admin/pembayaran');
    ...
    public function ajax_pembayaran_hapus()
    {
        ...
        $temp=$this->db1->ajax_pembayaran_hapus();
    ...
    {
        $data = $this->check();
        $data['cetak']=$this->db1->ajax_print_all();
        $cetak = $this->load->view('page/print_all', $data);
        $cetak = $cetak->output->final_output;
        echo json_encode(array("cetak" => $cetak, "csrf" => array("name" => $this->security->get_csrf_token_name(), "hash" => $this->security->get_csrf_hash())));
        exit();
    }
